      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 lg:py-20 z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center">
          ...
          <div className="space-y-6 sm:space-y-8">
            <div className="space-y-4 sm:space-y-6">
              <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold leading-tight text-white">
                ...
                <span className="block text-yellow-300 text-xl sm:text-2xl md:text-3xl lg:text-4xl xl:text-5xl">
                  {language === 'th' ? 'และอุทธรณ์คำสั่งปฏิเสธวีซ่า' : language === 'en' ? '& Visa Appeal' : '和签证上诉'}
                </span>
                <span className="block text-blue-200 text-lg sm:text-xl md:text-2xl lg:text-3xl xl:text-4xl mt-1 sm:mt-2">
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
              <a
                href="#booking"
                className="group inline-flex items-center justify-center px-6 sm:px-8 py-3 sm:py-4 bg-white text-blue-900 font-bold rounded-xl hover:bg-blue-50 transition-all duration-300 shadow-xl hover:shadow-2xl transform hover:-translate-y-1"
              >
                <span className="text-base sm:text-lg">{t.hero.cta1}</span>
              ...
              <a
                ...
                className="group inline-flex items-center justify-center px-6 sm:px-8 py-3 sm:py-4 bg-green-600 text-white font-bold rounded-xl hover:bg-green-700 transition-all duration-300 shadow-xl hover:shadow-2xl transform hover:-translate-y-1"
              >
                <MessageCircle className="mr-2 sm:mr-3 w-5 h-5 sm:w-6 sm:h-6 group-hover:scale-110 transition-transform duration-200" />
                <span className="text-base sm:text-lg">{t.hero.cta2}</span>
              </a>
            </div>

            {/* Trust Indicators */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 pt-6 sm:pt-8">
              <div className="text-center bg-white/10 backdrop-blur-sm rounded-xl p-4 sm:p-6 border border-white/20">
                <div className="text-3xl sm:text-4xl font-bold text-yellow-300 mb-1 sm:mb-2">20+</div>
                <div className="text-blue-100 font-medium text-sm sm:text-base">
                  {language === 'th' ? 'ปีประสบการณ์' : language === 'en' ? 'Years Experience' : '年经验'}
                </div>
              </div>
              <div className="text-center bg-white/10 backdrop-blur-sm rounded-xl p-4 sm:p-6 border border-white/20">
                <div className="text-3xl sm:text-4xl font-bold text-yellow-300 mb-1 sm:mb-2">500+</div>
                <div className="text-blue-100 font-medium text-sm sm:text-base">
                  {language === 'th' ? 'คดีที่ประสบความสำเร็จ' : language === 'en' ? 'Successful Cases' : '成功案例'}
                </div>
              </div>
              <div className="text-center bg-white/10 backdrop-blur-sm rounded-xl p-4 sm:p-6 border border-white/20">
                <div className="text-3xl sm:text-4xl font-bold text-yellow-300 mb-1 sm:mb-2">95%</div>
                <div className="text-blue-100 font-medium text-sm sm:text-base">
          ...
          <div className="relative mt-8 lg:mt-0">
            <div className="bg-white/15 backdrop-blur-md rounded-2xl sm:rounded-3xl p-6 sm:p-8 border border-white/30 shadow-2xl">
              <div className="space-y-6 sm:space-y-8">
                <div className="text-center">
                  <div className="relative mb-4 sm:mb-6">
                    <div className="w-20 h-20 sm:w-24 sm:h-24 bg-gradient-to-br from-yellow-400 to-yellow-500 rounded-full flex items-center justify-center mx-auto shadow-lg">
                ...
                <div className="space-y-3 sm:space-y-4">
                  <div className="flex items-center space-x-3 sm:space-x-4 p-3 bg-white/10 rounded-lg">
